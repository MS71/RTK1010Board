...
static char my_wifi_ssid[64] = CONFIG_WIFI_SSID;
static char my_wifi_psk[64] = CONFIG_WIFI_PASSWORD;
...
static void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    else if(event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        ESP_LOGW(TAG, "SYSTEM_EVENT_STA_GOT_IP %d.%d.%d.%d", (s_ip_addr.addr >> 0) & 0xff, (s_ip_addr.addr >> 8) & 0xff,
        ...
        ESP_LOGW(TAG, "wifi connected rssi=%d ssid=%s bssid=%02x:%02x:%02x:%02x:%02x:%02x channel=%d|%d", ap_info.rssi,
            ap_info.ssid, ap_info.bssid[0], ap_info.bssid[1], ap_info.bssid[2], ap_info.bssid[3], ap_info.bssid[4],
            ap_info.bssid[5], ap_info.primary, ap_info.second);
#endif
    }
    else if(event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_BEACON_TIMEOUT)
    {
        ESP_LOGW(TAG, "unhandled WIFI_EVENT event_id=WIFI_EVENT_STA_BEACON_TIMEOUT");
    }
    else if(event_base == WIFI_EVENT)
    {
        ESP_LOGW(TAG, "unhandled WIFI_EVENT event_id=0x%08x", (int)event_id);
    }
    else if(event_base == IP_EVENT)
    {
        ESP_LOGW(TAG, "unhandled IP_EVENT event_id=0x%08x", (int)event_id);
    }
    else
    {
